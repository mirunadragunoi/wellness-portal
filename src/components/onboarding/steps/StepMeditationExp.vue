<template>
  <div class="step-select">
    ...
    <div class="options">
      ...
      >
        <Icon :icon="ONBOARDING_MEDITATION_EXP_ICONS[opt.id]" class="option__icon app-icon app-icon--md app-icon--primary" />
...
import { ONBOARDING_MEDITATION_EXP_ICONS } from '@/constants/appIcons'

const { t } = useI18n()
const emit = defineEmits(['next'])
const selected = ref('')
const options = [
  { id: 'new',        labelKey: 'onboarding.exp_new'        },
  { id: 'tried',      labelKey: 'onboarding.exp_tried'      },
  { id: 'occasional', labelKey: 'onboarding.exp_occasional' },
  { id: 'regular',    labelKey: 'onboarding.exp_regular'    }
]
</script>
