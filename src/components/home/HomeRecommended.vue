<template>
  <div class="recommended">
    <div class="recommended__header">
      ...
      <RouterLink to="/explore" class="recommended__see-all">{{ t('home.see_all') }}</RouterLink>
    </div>
      ...
      <div class="recommended__track">
        ...
        >
          <div class="session-card__img" :style="{ background: session.thumbnailGradient }">
            ...
            <button class="session-card__play">▶</button>
            <span class="session-card__type-icon">{{ typeEmoji(session.type) }}</span>
          </div>
...
function typeEmoji(type) {
  const map = { meditation: '🧘', 'sleep-story': '🌙', soundscape: '🎵', motivational: '⚡', breathing: '💨' }
  return map[type] || '🧘'
}
...
.recommended__see-all { font-size: 14px; color: var(--sky-600); text-decoration: none; font-weight: 500; }
...
.session-card__play {
  ...
  padding-left: 2px; box-shadow: 0 2px 8px rgba(0,0,0,0.15);
  transition: transform var(--duration-fast) var(--ease-bounce);
}
.session-card:hover .session-card__play { transform: scale(1.15); }
.session-card__type-icon { position: absolute; bottom: 10px; right: 10px; font-size: 16px; }
